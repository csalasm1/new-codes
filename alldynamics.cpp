...
int main (void)
{
  const double N  = 6;
  const double TA = 0;
  const double TB = 1000000;
  const double H  = 0.1;
  std::vector<double> y = {2.5,0,0 ,0, 1.68884, 0.2}; // {x0, y0, z0, vx0, vy0, vz0}  ---1.707---
    rk4(TA,TB, H, y);
...
/*void euler(double ta, double tb, double h, std::vector<double> & y)
{
 ...
 void rk4(double ta, double tb, double h, std::vector<double> & y)
 {
   ...
   const double eps= 0.11;
   const double epsn= -0.11;
   double sum=0.0;


    std::ofstream myfile;
  myfile.open ("poincaire.txt");
   const int N = (tb-ta)/h;
   ...
   for (int nt = 0; nt < N; ++nt) {
    ...
    // myfile << y[0] << "\t" << y[1] << "\t" << y[2]<< "\t" << y[3] <<  "\t" << y[4] <<  "\t" << y[5] <<  "\n";
 ...
 double f(double t, const std::vector<double> & y, int id)
 {
   if (0 == id) {
     ...
     return (O*y[4])-A*(2*y[0]/(a*a))*(1/(C+((y[0]*y[0])/(a*a))+((y[1]*y[1])/(b*b))+((y[2]*y[2])/(c*c))));
   }
   else if (4 == id){
     return -(O*y[3])-A*(2*y[1]/(b*b))*(1/(C+((y[0]*y[0])/(a*a))+((y[1]*y[1])/(b*b))+((y[2]*y[2])/(c*c))));
   }
   else if (5 == id){
     return -A*(2*y[2]/(c*c))*(1/(C+((y[0]*y[0])/(a*a))+((y[1]*y[1])/(b*b))+((y[2]*y[2])/(c*c))));
   }
 }
